path <- setwd("C:/Users/P/OneDrive - Aalborg Universitet/Skrivebord/P10")
source("Get_Data_Hourly.R")

options(contrasts = c("contr.treatment", "contr.poly"))

######################
#### FACTOR MODEL ####
######################
df <- train_pdata %>%
  select(
    Date, Hour,
    LogPrice,
    SpotPrice_DK2, SpotPrice_DE, SpotPrice_NO2, SpotPrice_SE3, SpotPrice_SE4, #Weekday, Month,
    Consumption_HS, Consumption_MJ, Consumption_NJ, Consumption_SJ, Consumption_SJL,
    OffshoreWindPower_DK1, OffshoreWindPower_DK2,
    OnshoreWindPower_DK1, OnshoreWindPower_DK2,
    SolarPower_DK1, SolarPower_DK2,
    ProductionLt100MW, ProductionGe100MW,
    ExchangeGermany, ExchangeNetherlands,
    ExchangeGreatBritain, ExchangeNorway, ExchangeSweden,
    BornholmSE4
  ) %>%
  na.omit()
y <- df$LogPrice
X_raw <- df %>% select(-Date, -Hour, -LogPrice)

cor_matrix <- cor(X_raw)

round(cor_matrix, 2)

cor_matrix <- cor(X_raw)

png("hourly_correlation_plot.png", width = 2000, height = 2000, res = 300)

corrplot(
  cor_matrix,
  method = "color",
  type = "upper",
  # order = "hclust",
  addCoef.col = "black",
  number.cex = 0.4,
  tl.cex = 0.5,
  tl.col = "black",
  tl.srt = 45,
  col = colorRampPalette(c("#B2182B", "white", "#2166AC"))(200),
  diag = FALSE
)
dev.off()

# STEP 1 — PCA on X_i (choose factors)
X_scaled <- scale(X_raw)

pca_X <- prcomp(X_scaled, center = TRUE, scale. = TRUE)

png("hourly_scree_plot_X.png", width = 2000, height = 1400, res = 300)
plot(pca_X, type = "l")   # scree plot
dev.off()

pca_sum <- summary(pca_X)
pca_sum
pca_table <- data.frame(
  # PC = paste0("PC", 1:6),
  Std_Dev = pca_sum$importance["Standard deviation", 1:6],
  Prop_Var = pca_sum$importance["Proportion of Variance", 1:6],
  Cum_Prop = pca_sum$importance["Cumulative Proportion", 1:6]
)


latex_code <- kable(
  pca_table,
  format = "latex",
  booktabs = TRUE,
  caption = "PCA Summary on X (First 6 Principal Components) on hourly data",
  label = "tab:hourly_pca_table_X"
)

writeLines(latex_code, "hourly_pca_table_X.tex")


r_x <- 4
F_X <- pca_X$x[, 1:r_x, drop = FALSE]

# STEP 2 — OLS estimation of Y_i − X_i β
df_stage1 <- data.frame(
  df[, c("Date", "Hour")],
  y = y,
  F_X
)

ols_stage1 <- lm(y ~ F_X, data = df_stage1)

df_stage1$residuals_1 <- residuals(ols_stage1)

# STEP 3 — PCA on residual structure → \widehat{F}^e
res_matrix <- reshape2::acast(
  df_stage1,
  Date ~ Hour,
  value.var = "residuals_1"
)

pca_res <- prcomp(res_matrix, center = TRUE, scale. = TRUE)

png("hourly_scree_plot_residuals.png", width = 2000, height = 1400, res = 300)
plot(pca_res, type = "l")
dev.off()

pca_res_sum <- summary(pca_res)

pca_table <- data.frame(
  # PC = paste0("PC", 1:6),
  Std_Dev = pca_res_sum$importance["Standard deviation", 1:6],
  Prop_Var = pca_res_sum$importance["Proportion of Variance", 1:6],
  Cum_Prop = pca_res_sum$importance["Cumulative Proportion", 1:6]
)


latex_code <- kable(
  pca_table,
  format = "latex",
  booktabs = TRUE,
  caption = "PCA Summary on the residuals (First 6 Principal Components)",
  label = "tab:hourly_pca_table_res"
)

writeLines(latex_code, "hourly_pca_table_res.tex")


r_e <- 2
F_e <- pca_res$x[, 1:r_e, drop = FALSE]

# STEP 4 — Construct moment condition
Z <- cbind(Intercept = 1, F_X)
Z <- as.matrix(Z)

# Compute residuals (aligned)
u_hat <- df_stage1$residuals_1

moment_part <- colMeans(Z * u_hat) # 1/N ∑Z_i' u_i

# STEP 4b — Build correction term S(F^e ⊗ Id)g
# correction <- F_e
# align dimensions: map F_e into same dimension as Z moments
g_hat <- colMeans(F_e)
correction <- rep(g_hat, length.out = length(moment_part))
names(correction) <- names(moment_part)

# FINAL STEP — estimator
mu_bar <- moment_part - correction
mu_bar


gmm_moments <- function(theta, y, Z) {

  # Z = (Intercept, F_X)
  # theta = (alpha, beta_1, ..., beta_r)

  u <- y - Z %*% theta

  g <- colMeans(Z * as.numeric(u))

  return(g)
}

gmm_objective <- function(theta, y, Z, W) {

  g <- gmm_moments(theta, y, Z)

  as.numeric(t(g) %*% W %*% g)
}

init <- coef(lm(y ~ F_X))
init <- as.numeric(init)

names(init) <- colnames(Z)

W <- diag(ncol(Z))

gmm_fit <- optim(
  par = init,
  fn = gmm_objective,
  y = y,
  Z = Z,
  W = W,
  method = "BFGS"
)
theta_hat <- gmm_fit$par
theta_hat


#########################
#### FORECAST FACTOR ####
#########################
h <- 5
T <- nrow(res_matrix)
N <- ncol(res_matrix)

# Forecast X-factors F_X
F_X_ts <- as.data.frame(F_X)

F_X_fcast <- sapply(F_X_ts, function(x) {
  forecast(auto.arima(x), h = h)$mean
})

F_X_fcast <- as.matrix(F_X_fcast)   # h × r_x

# Forecast residual factors F_e
F_e_ts <- as.data.frame(F_e)

F_e_fcast <- sapply(F_e_ts, function(x) {
  forecast(auto.arima(x), h = h)$mean
})

F_e_fcast <- as.matrix(F_e_fcast)   # h × r_e

# lambda_hat <- matrix(colMeans(F_e), nrow = N, ncol = r_e, byrow = TRUE)
lambda_hat <- solve(t(F_e) %*% F_e) %*% t(F_e) %*% res_matrix
lambda_hat <- as.matrix(lambda_hat)   # r_e × N

beta_lambda <- lambda_hat

beta_hat <- coef(ols_stage1)
beta_hat <- as.numeric(beta_hat)

F_X_fcast_mat <- as.matrix(F_X_fcast)

common_forecast <- F_X_fcast_mat %*% beta_hat[-1] + beta_hat[1]
common_forecast_mat <- matrix(common_forecast, nrow = h, ncol = N)

# Forecast residual component:
residual_forecast <- F_e_fcast %*% lambda_hat

final_forecast <- common_forecast_mat + residual_forecast
colnames(final_forecast) <- colnames(res_matrix)
rownames(final_forecast) <- paste0(
  "Day_", 1:h
)
final_forecast

# start of forecast horizon
start_date <- as.Date("2026-05-01")

forecast_long <- as.data.frame(final_forecast) %>%
  mutate(Day = rownames(final_forecast)) %>%
  pivot_longer(
    cols = -Day,
    names_to = "Hour",
    values_to = "LogPrice"
  ) %>%
  group_by(Day) %>%
  mutate(
    Hour = row_number() - 1   # ← HERE
  ) %>%
  ungroup() %>%
  mutate(
    Date = as.Date(start_date + as.integer(substr(Day, 5, 5)) - 1)
  ) %>%
  select(Date, Hour, LogPrice)

# forecast_long$Forecast_Price <- predict(yj, newdata = forecast_long$LogPrice_100,
#                                         inverse = TRUE)

forecast_long$Forecast_Price <- exp(forecast_long$LogPrice) -
  (abs(min(train_data$SpotPrice_DK1, na.rm = TRUE)) + 100)

observed_data <- data.frame(
  Date = as.Date(as.character(c(train_pdata$Date,
                                test_pdata$Date))),

  Hour = as.numeric(as.character(c(train_pdata$Hour,
                                   test_pdata$Hour))),

  SpotPrice_DK1 = c(train_pdata$SpotPrice_DK1,
                    test_pdata$SpotPrice_DK1)
)

plot_data <- observed_data %>%
  full_join(
    forecast_long %>% select(-LogPrice),
    by = c("Date", "Hour")
  ) %>%
  arrange(Date, Hour) %>%
  filter(
    Date >= as.Date("2026-04-20"),
    Date <= as.Date("2026-05-05")
  )

eval_data <- plot_data %>%
  filter(!is.na(Forecast_Price))

rmse_full <- sqrt(mean((eval_data$SpotPrice_DK1 - eval_data$Forecast_Price)^2))

mae_full <- mean(abs(eval_data$SpotPrice_DK1 - eval_data$Forecast_Price))

forecast_no_resid <- common_forecast_mat
eval_data$Forecast_NoResid <- as.vector(forecast_no_resid)

rmse_restricted <- sqrt(mean((eval_data$SpotPrice_DK1 - eval_data$Forecast_NoResid)^2))

mae_restricted <- mean(abs(eval_data$SpotPrice_DK1 - eval_data$Forecast_NoResid))

improvement_rmse <- (rmse_restricted - rmse_full) / rmse_restricted * 100
improvement_mae  <- (mae_restricted - mae_full) / mae_restricted * 100

results_table <- data.frame(
  Model = c("Full model",
            "Restricted model",
            "Improvement (%)"),
  RMSE = c(rmse_full, rmse_restricted, improvement_rmse),
  MAE  = c(mae_full, mae_restricted, improvement_mae)
)

latex_code <- kable(
  results_table,
  format = "latex",
  booktabs = TRUE,
  digits = 3,
  caption = "Forecast accuracy comparison between full factor model and restricted model.",
  label = "tab:hourly_forecast_accuracy"
)

writeLines(latex_code, "hourly_forecast_accuracy.tex")

plot_Hour_grid <- function(data, hours, save = FALSE) {

  # filter selected hours
  df <- data %>%
    filter(Hour %in% hours) %>%
    arrange(Date, Hour)

  # create connected forecast series
  df <- df %>%
    group_by(Hour) %>%
    arrange(Date) %>%
    mutate(
      Forecast_Connected = Forecast_Price
    )

  # forecast start
  forecast_start <- min(df$Date[!is.na(df$Forecast_Price)])

  # connect forecast to last observed point
  df <- df %>%
    group_by(Hour) %>%
    mutate(
      Forecast_Connected = ifelse(
        Date == forecast_start - 1,
        SpotPrice_DK1,
        Forecast_Connected
      )
    ) %>%
    ungroup()

  #
  # # detect forecast start date
  # forecast_start <- df %>%
  #   filter(!is.na(Forecast_Price)) %>%
  #   summarise(start_date = min(Date)) %>%
  #   pull(start_date)
  #
  # reshape to long format
  df_long <- df %>%
    pivot_longer(
      cols = c(SpotPrice_DK1, Forecast_Connected),
      names_to = "Type",
      values_to = "Value"
    ) %>%
    mutate(
      Type = dplyr::recode(
        Type,
        SpotPrice_DK1 = "Observed",
        Forecast_Connected = "Forecast"
      )
    )

  # plot
  p <- ggplot(df_long, aes(x = Date, y = Value, color = Type)) +

    geom_vline(
      xintercept = forecast_start - 1,
      linetype = "dashed",
      color = "black",
      linewidth = 0.6
    ) +

    geom_line(linewidth = 0.4, na.rm = TRUE) +

    facet_wrap(~Hour, ncol = 4, scales = "free_y") +

    scale_color_manual(
      values = c(
        "Observed" = "black",
        "Forecast" = "blue"
      )
    ) +

    labs(
      x = NULL,
      y = NULL,
      color = NULL
    ) +

    theme_minimal() +
    theme(
      strip.text = element_text(size = 10),
      legend.position = "bottom"
    )

  filename <- paste0(
    "HourlyForecast_H",
    min(hours),
    "to",
    max(hours),
    ".png"
  )

  if (save) {
    ggsave(
      filename = filename,
      plot = p,
      width = 10,
      height = 6,
      dpi = 600
    )
  }

  return(p)
}
# Example
plot_Hour_grid(plot_data, hours = 0:11) # 0-11
plot_Hour_grid(plot_data, hours = 12:23) # 11-23
plot_Hour_grid(plot_data, hours = 0:23, TRUE)


plot_data <- plot_data %>%
  mutate(
    DateTime = as.POSIXct(Date) + Hour * 3600
  ) %>%
  arrange(DateTime)


forecast_start <- min(
  plot_data$DateTime[!is.na(plot_data$Forecast_Price)]
)

plot_data <- plot_data %>%
  mutate(
    Forecast_Connected = Forecast_Price
  )

last_observed <- forecast_start - 3600

plot_data$Forecast_Connected[
  plot_data$DateTime == last_observed
] <-
  plot_data$SpotPrice_DK1[
    plot_data$DateTime == last_observed
  ]

plot_full_forecast <- function(data, save = FALSE) {

  forecast_start <- min(
    data$DateTime[!is.na(data$Forecast_Price)]
  )

  # Historical data only BEFORE forecast
  historical_df <- data %>%
    filter(DateTime < forecast_start)

  # Actual realized values AFTER forecast start
  actual_df <- data %>%
    filter(DateTime >= forecast_start)

  # Forecast values
  forecast_df <- data %>%
    filter(!is.na(Forecast_Connected))

  p <- ggplot() +

    # Historical
    geom_line(
      data = historical_df,
      aes(DateTime, SpotPrice_DK1,
          color = "Historical"),
      linewidth = 0.4
    ) +

    # Actual future realizations
    geom_line(
      data = actual_df,
      aes(DateTime, SpotPrice_DK1,
          color = "Actual"),
      linewidth = 0.4,
      linetype = "dotted"
    ) +

    # Forecast
    geom_line(
      data = forecast_df,
      aes(DateTime, Forecast_Connected,
          color = "Forecast"),
      linewidth = 0.5
    ) +

    geom_vline(
      xintercept = forecast_start,
      linetype = "dashed",
      color = "black",
      linewidth = 0.5
    ) +

    scale_color_manual(
      values = c(
        "Historical" = "black",
        "Forecast" = "red",
        "Actual" = "blue"
      )
    ) +

    labs(
      title = "Factor Model Forecast vs Actual for Hourly Data",
      x = "Date",
      y = "Spot Price",
      color = NULL
    ) +

    theme_minimal() +

    theme(
      plot.title = element_text(
        hjust = 0.5,
        size = 16
      ),

      axis.title = element_text(size = 14),

      legend.position = "right",

      legend.text = element_text(size = 11)
    )

  if(save) {

    ggsave(
      "hourly_full_forecast.png",
      p,
      width = 12,
      height = 6,
      dpi = 600
    )

  }

  return(p)
}

plot_full_forecast(plot_data, TRUE)


#################
# TESTS
#################
IC_X <- ICr(X_scaled)
IC_X

IC_res <- ICr(res_matrix)
IC_res

Box.test(F_X[,1], type = "Ljung-Box")

checkresiduals(auto.arima(F_X[,1]))

adf.test(F_X[,1])
adf.test(F_e[,1])

kpss.test(F_X[,1])

qqnorm(residuals(ols_stage1), col = "blue")
qqline(residuals(ols_stage1), col = "red")
