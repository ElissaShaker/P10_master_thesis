...
forecast_panel <- function(panel_df,
  ...
  alpha_i <- fe[as.character(unique(panel_df$Quarter))]
  ...
  data.frame(
    Quarter = unique(panel_df$Quarter),
    Date = future_dates,
...
forecast_df$Quarter <- as.numeric(
  as.character(forecast_df$Quarter)
)

# =========================================================
# 9. Add time information
# =========================================================
...
forecast_df <- forecast_df %>%
  left_join(quarter_lookup, by = "Quarter")
...
ggplot() +
  ...
  geom_line(
    ...
    aes(DateTime, ForecastPrice, color = "Forecast"),
    linewidth = 0.7
  ) +
  ...
  # Forecast origin
  # geom_vline(
  #   xintercept = as.POSIXct(min(forecast_plot$DateTime)),
  #   linetype = "dashed",
  #   color = "darkgreen",
  #   linewidth = 0.7
  # ) +
  ...
  labs(
    title = "Dynamic Panel Forecast vs Actual for Quarterly data",
    x = "Date",
    y = "Spot Price",
    color = ""
  ) +
...
plot_dynamic_panel_forecast <- function(
  ...
  if(save) {
    ...
    ggsave(
      filename = "quarterly_DynamicPanelForecast_full.png",
      plot = p,
      width = 12,
      height = 6,
      dpi = 600
...
plot_quarter_grid <- function(
  ...
  p <- ggplot(
    ...
    labs(
      title = "Quarter-Specific Forecast Validation",
      x = NULL,
      y = NULL,
      color = NULL
    ) +
